...
class ViewImageController < Sinatra::Base
  ...
  get %r{/thumbnail/(\d+x\d+)/(\d+x\d+)/(\d+)\.(jpg|png|jpeg|gif)} do |width_by_height, aspect_ratio, id, image_type|
    ...
    @aspect_ratio = IMAGE_ASPECT_RATIOS[aspect_ratio] || 1
    
    image_binary = ImageBinary.find(:id => id)
    if(image_binary)
      buffered_image = ImageIOUtils.to_buffered_image(image_binary.file.to_java_bytes)
      ...
      status 404
      content_type :json
      { :status => 404, :message => "The image you requested with id=#{id} was not found." }.to_json
    end
